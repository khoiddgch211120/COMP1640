...
public class DashboardServiceImpl implements DashboardService {
    ...
    private DashboardResponse buildOverall() {
        ...
        Integer ideasThisYear = academicYearRepository.findCurrent(LocalDate.now())
    ...
    private List<DashboardResponse.DeptBreakdown> buildDeptBreakdown(
        ...
        return depts.stream()
                .map(dept -> {
                    List<Idea> deptIdeas = allIdeas.stream()
                            .filter(i -> i.getDepartment() != null
                                      && i.getDepartment().getDeptId().equals(dept.getDeptId()))
                            .collect(Collectors.toList());
                    ...
                    long userCount = userRepository.findByDepartment_DeptId(dept.getDeptId()).size();
                ...
                .sorted(Comparator.comparingInt(DashboardResponse.DeptBreakdown::getIdeaCount).reversed())
                .collect(Collectors.toList());
    }
}