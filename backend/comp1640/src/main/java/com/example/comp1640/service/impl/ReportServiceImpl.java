...
public class ReportServiceImpl implements ReportService {
      ...
      public List<StatisticsReportResponse> getStatisticsReport(Integer yearId, Integer deptId) {
            ...
            // Denominator must always be total ideas for the year (all departments),
            // not just the filtered subset — to avoid percentage always being 100%.
            long totalIdeas;
            if (yearId != null) {
                  totalIdeas = ideaRepository.findByAcademicYear_YearId(yearId).size();
            } else {
            ...
            Map<Integer, List<Idea>> ideaByDept = ideas.stream()
                        .collect(Collectors
                                    .groupingBy(i -> i.getDepartment() != null ? i.getDepartment().getDeptId() : -1));
      ...
      public List<AnonymousContentResponse> getAnonymousContent(Integer yearId) {
            ...
            anonymousComments.forEach(comment -> {
                  anonymousContents.add(AnonymousContentResponse.builder()
                              ...
                              .contentPreview(comment.getContent().substring(0,
                                          Math.min(100, comment.getContent().length())))
            ...
            List<TopContributorDto> topContributors = ideas.stream()
                        ...
                        .map(entry -> {
                              ...
                              return TopContributorDto.builder()
                                          .userId(entry.getKey())
                                          .fullName(
                                                      userIdeas.get(0).getUser() != null
                                                                  ? userIdeas.get(0).getUser().getFullName()
                                                                  : "Unknown")
                                          .deptName(userIdeas.get(0).getDepartment() != null
                                                      ? userIdeas.get(0).getDepartment().getDeptName()
                                                      : "")
