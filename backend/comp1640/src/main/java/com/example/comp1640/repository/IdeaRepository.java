...
public interface IdeaRepository extends JpaRepository<Idea, Integer> {
        ...
        // Filtered by isDisabled=false
        Page<Idea> findByAcademicYear_YearIdAndDepartment_DeptIdAndIsDisabledFalse(Integer yearId, Integer deptId, Pageable pageable);

        Page<Idea> findByAcademicYear_YearIdAndIsDisabledFalse(Integer yearId, Pageable pageable);

        Page<Idea> findByDepartment_DeptIdAndIsDisabledFalse(Integer deptId, Pageable pageable);

        Page<Idea> findByIsDisabledFalse(Pageable pageable);
        ...
        @Modifying
        @Query(value = "DELETE FROM idea_category WHERE idea_id IN (SELECT idea_id FROM idea WHERE user_id = :userId)", nativeQuery = true)
        void deleteIdeaCategoriesByUserUserId(@Param("userId") Integer userId);

        @Modifying
        @Query("DELETE FROM Idea i WHERE i.user.userId = :userId")
